function loadSettings(type){
  (function aaa(){
    if(!$(".rightFilter a").eq(0).attr("href")){
      ...
      let check = type != pathname ? $(type).attr("id").slice(0, -9) : hash ? hash : $(".rightFilter a").eq(0).attr("href").split("#")[1];
      hash = check;
      $(`.rightFilter input#${check}FilterMax`).prop("checked", true);
      history.replaceState('', null, pathname+"#"+check);
      $("main ul").html(`
        <div class="reset" name="${translate(["menu", "filter", "reset"])}" onclick="reset(this)"></div>
      `);
